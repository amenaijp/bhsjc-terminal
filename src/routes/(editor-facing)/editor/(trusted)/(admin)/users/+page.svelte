<script lang="ts">
	import type { PageData } from './$types';
	import { enhance } from '$app/forms';

	let { data }: { data: PageData } = $props();
</script>

<p>Displaying {data.users.length} user(s)</p>
<form method="GET">
	<label class="flex flex-row items-center gap-2">
		<input
			type="checkbox"
			name="unverified"
			value="true"
			checked={data.onlyUnverified}
			onchange={(e) => (e.currentTarget as HTMLInputElement).form?.submit()}
		/>
		Only show unverified users
	</label>
</form>
<p>
	Anyone can register with any email address. If they haven't verified their email, be careful
	promoting them to verified.
</p>
<div class="mt-6 overflow-x-auto">
	<table class="w-fit border-collapse">
		<thead>
			<tr class="border-b border-[#E0E0E0]">
				<th class="px-2 pb-2 text-left">Full Name</th>
				<th class="px-2 pb-2 text-left">Email Address</th>
				<th class="px-2 pb-2 text-left">Email Verified?</th>
				<th class="px-2 pb-2 text-left">User Type</th>
				<th class="px-2 pb-2 text-left">Promote</th>
				<th class="px-2 pb-2 text-left">Created at (dd/mm/yyyy, en-NZ)</th>
				<th class="px-2 pb-2 text-left">Delete</th>
			</tr>
		</thead>
		<tbody>
			{#each data.users as user}
				<tr class="hover:bg-gray-50">
					<td class="px-2 py-1">{user.name}</td>
					<td class="px-2 py-1">{user.email}</td>
					<td class="px-2 py-1" class:bg-red-100={!user.emailVerified}
						>{user.emailVerified ? 'verified' : 'NOT verified'}</td
					>
					<td class="px-2 py-1">{user.userType}</td>
					<td class="px-2 py-1">
						{#if user.userType !== 'admin'}
							<form method="POST" action="?/promote" use:enhance>
								<input type="hidden" name="id" value={user.id} />
								{#if user.userType === 'unverified'}
									<input type="hidden" name="to" value="editor" />
									<button
										class="rounded-sm bg-blue-600 px-2 py-1 text-white transition hover:bg-blue-700"
										>Make Editor</button
									>
								{:else if user.userType === 'editor'}
									<input type="hidden" name="to" value="admin" />
									<button
										class="rounded-sm bg-blue-600 px-2 py-1 text-white transition hover:bg-blue-700"
										>Make Admin</button
									>
								{/if}
							</form>
						{:else}
							<p class="text-gray-400">already admin</p>
						{/if}
					</td>
					<td class="px-2 py-1"
						>{user.createdAt.toLocaleString('en-NZ', {
							dateStyle: 'short',
							timeStyle: 'short'
						})}</td
					>
					<td class="px-2 py-1">
						{#if user.userType === 'unverified'}
							<form method="POST" action="?/delete" use:enhance>
								<input type="hidden" name="id" value={user.id} />
								<button
									class="rounded-sm bg-red-600 px-2 py-1 text-white transition hover:bg-red-700"
									>Delete</button
								>
							</form>
						{:else}
							<p class="text-gray-400">can't delete editors/admins</p>
						{/if}
					</td>
				</tr>
			{/each}
		</tbody>
	</table>
</div>
